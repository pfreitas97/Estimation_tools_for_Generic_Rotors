...
# Testing regression models for airfoil lift slope curve

import scipy.stats


#Convenient for interacting with airfoil data from xfoil.
...
def _getAlphaZL(airfoil):
    ''' Find the angle for which CL = 0, i.e. the angle of zero lift
    
    Parameters:
    airfoil: Pandas Dataframe with format: [Alpha,CL,CD] where Alpha means angle of attack
        
    Returns Angle in Radians.
    '''
    
    idx = np.abs(airfoil['CL'].to_numpy(dtype=float)).argmin()
    
    return np.deg2rad(float(airfoil.iloc[idx,0]))



def _getCollectivePitch(twist,alphaZL,twist_provided = True):
    ...
    if twist is not provided otherwise it will assume the twist matrix provided includes the collective value.
    
    Parameters:
        twist: twist vector in Radians
        alphaZL: The zero lift angle of attack in Radians.
        twist provided: Boolean, changes the collective chosen if a twist matrix was included.
    '''
    if twist_provided:
        collective = 0
        pass
...
def hoverPerformance_Classical(b,R,C,omega,airfoil,rR = [],cR = [], twist = []):
    ...
    alphaZL = _getAlphaZL(airfoil)
    
    
    theta = _getCollectivePitch(twist, alphaZL,twist_provided)
...
def hoverPerformance_NoHeuristics(b,R,C,omega,airfoil,rR,cR, twist):
    ...
    attempts to correct for tip loss / hub loss. Hub position is assumed to be zero, and CT returned is explicitly 
    the calculated value with no heuritic modifications.'''
    
    
    CTnoLoss = _getCTnoTipLoss(runningThrustLoad, rR, x_hub=0)
    ...
    ''' B is set to 1, and x_hub is set to 0 in order to marginalize tip loss and rotor hub effects in return value'''
    
    CQi = _getInducedTorqueCoefficient(runningInducedTorque,rR,B=1,x_hub=0)
